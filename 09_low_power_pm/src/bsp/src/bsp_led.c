...
/**
 * @brief      Retrieves the raw Zephyr device pointer for the primary system PWM.
 * @param[in]  None.
 * @return     const struct device* Pointer to the PWM device instance.
 */
const struct device *bsp_pwm_get_dev(void)
{
    return pwm_leds[1].dev;
}
...
static int bsp_pwm_led_init(void)
{
    for (uint8_t i = 0; i < PWM_LED_NUM_MAX; i++)
    {
        ...
        bsp_set_individual_pwm_led(i, 0);
    }
