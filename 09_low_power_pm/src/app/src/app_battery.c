...
int app_battery_process_sample(void)
{
    ...
    if (adc_dev != NULL)
    {
        app_power_pm_toggle_device(adc_dev, false);
    }
    ...
    if (adc_dev != NULL)
    {
        (void)app_power_pm_toggle_device(adc_dev, true);
    }
    ...
    LOG_DBG("Battery Telemetry - Measured: %u mV, Filtered: %u mV, SoC: %u%%", fresh_raw_mv, current_filtered_mv, app_battery_get_soc(current_filtered_mv));
...
uint8_t app_battery_get_soc(uint16_t voltage_mv)
{
    ...
    /* Linearly map the voltage input insapp_battery_get_socide the safe operational cell envelope */
